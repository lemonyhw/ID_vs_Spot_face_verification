...
from utils.evaluate_prototype_1 import eval
...
parser.add_argument('--pre_model', type=str, default='./models/checkpoint_NJN_RP_AL_npair/DummpyFace_20_checkpoint.pth')
...
parser.add_argument('--classifier_type', type=str, default='DUM', help='Which classifier for train. (MCP, AL, L,ARC,TRIP,DUM)')
...
parser.add_argument('--lr', type=float, default=0.000001, help='learning rate (default: 0.1)')
...
parser.add_argument('--save_path', type=str, default='./models/checkpoint_NJN_RP_DUM/',help='path to save checkpoint')
...
def main():
    ...
    # data = np.load(args.new_weight)
    # print("new_weight",data.shape)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for subpart in range(int(train_num / args.num_class)):
            ...
            # optimizer = torch.optim.SGD([{'params': model.parameters(),'lr':args.lr}, {'params': classifier.parameters(),'lr':args.lr}],
            #                             lr=args.lr,
            ...
            new_weight_[sub_train_label,:] = temp
