...
from utils.dataset import TripletList
...
parser.add_argument('--save_path', type=str, default='./models/checkpoint_NJN_tripletloss/',help='path to save checkpoint')
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(TripletList(
        root = args.train_root, train_list=args.train_list,phase="train",transform=train_transform),
    ...
    criterion = torch.nn.TripletMarginLoss(margin=2.0, swap=True).to(device)
    ...
    for epoch in range(1,args.epochs + 1):
        train(train_loader, model,model_eval,optimizer,criterion,epoch)
