...
parser.add_argument('--pre_model', type=str, default='./models/checkpoint_NJN_RP_DWI_AM_softmax/DummpyFace_13_checkpoint.pth')
parser.add_argument('--new_weight', type=str, default='./models/checkpoint_NJN_RP_DWI_AM_softmax/DummpyFace_13_checkpoint.mat'
                                                      '')
# Classifier
parser.add_argument('--num_class', type=int, default=100000, help='number of people(class)')
parser.add_argument('--classifier_type', type=str, default='DWI_AL', help='Which classifier for train. (MCP, AL, L,ARC,TRIP,DUM)')

#Extract feature
parser.add_argument('--prototype', type=str, default="ID", help='which feature for initializing weights')

# LR policy
parser.add_argument('--start_epoch', type=int, default=14, help='number of epochs to train (default: 30)')
...
parser.add_argument('--save_path', type=str, default='./models/checkpoint_NJN_RP_DWI_AM_softmax/',help='path to save checkpoint')
...
if args.database is "NJN":
    ...
    args.step_size = [5000, 10000, 15000]
else:
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        for subpart in range(int(train_num / args.num_class)):
            ...
            classifier = {
                ...
                "ARC": layer.ArcMarginProduct(512, args.num_class,sub_weight, s=30, m=0.3, easy_margin=True).to(device),
...
def train(train_loader,model,classifier, optimizer, epoch):
    ...
    for batch_idx, (data1, data2, _, target) in enumerate(train_loader):
        ...
        feats = model(data)
        output = classifier(feats,target)
